...
| `!version` | `!版本` | 查看版本資訊 |
| `!reboot` | `!重啟` | 重啟機器人（管理員） |
| `!afk 1h 睡覺了` | `!離開` | 設定限時 AFK 狀態 |
| `!小黑屋 @使用者 1h` | `!quarantine` | 暫時隱藏該使用者的所有頻道，時間到自動恢復 |
| `!使用者設定` | `!使用者面板`、`!個人面板` | 開關每日問候，並修改早安、午餐、晚間訊息 |
| `!玩 1m @使用者` | `!playwith` | 先邀請指定成員，等待對方回覆後由 AI 持續對話，時間到自動結束 |
