    ...
    <div class="container">
        ...
        <div class="glass-panel player-card">
            <div id="no-track" style="display:none;">
                ...
                <h2 style="color:var(--text-secondary)">?芸瘣迤?典?璈葉...</h2>
                <p style="color:var(--text-secondary)">暺??喳靘擐??改?</p>
            </div>
            <div id="player-content" style="width:100%; display:flex; flex-direction:column; align-items:center;">
                ...
                <div style="margin-top:40px; display:flex; align-items:center; gap:10px; padding:10px 20px; border-radius:50px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05);">
                    ...
                    <span id="requester-info" style="font-size:12px; font-weight:600; color:var(--text-secondary)">暺??? -</span>
                </div>
            </div>
        </div>
        ...
        <div class="sidebar">
            ...
            <div class="glass-panel sidebar-section">
                <div class="section-title"><i class="fas fa-signal"></i> 隤??</div>
                <div class="input-group">
                    <i class="fas fa-microphone-lines"></i>
                    <select id="vc-select">
                        <option value="">頛?駁?銝?..</option>
                    </select>
                </div>
                <button class="action-btn btn-primary" onclick="joinChannel()">
                    <i class="fas fa-plug"></i> ??駁?
                </button>
                <button class="action-btn btn-danger" onclick="leaveChannel()">
                    <i class="fas fa-sign-out-alt"></i> 霈?⊥???箄???                </button>
            </div>

            <!-- Search & Queue -->
            <div class="glass-panel sidebar-section" style="flex:1">
                <div class="section-title"><i class="fas fa-search"></i> ?單???</div>
                <div class="input-group">
                    <i class="fas fa-music"></i>
                    <input type="text" placeholder="頛詨?摮?銝?Enter..." id="search-input" onkeypress="if(event.key==='Enter') search()">
                </div>
                ...
                <div id="search-results" style="display:none; margin-bottom:20px; padding:10px; background:rgba(0,0,0,0.2); border-radius:16px; border:1px solid rgba(255,255,255,0.05);">
                    <div class="section-title" style="font-size:12px;"><i class="fas fa-list-check"></i> 隢????/div>
                    <div id="results-list"></div>
                    <button class="action-btn btn-danger" style="margin-top:10px; padding:8px; font-size:12px;" onclick="closeSearch()">????</button>
                </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function update() {
            ...
            if(!data.playing) {
                ...
                document.getElementById('requester-info').innerText = '暺??? ' + track.requester;
        ...
        async function leaveChannel() {
            if(!confirm('蝣箏?閬??芸瘣???喲??嚗?)) return;
        ...
        async function search() {
            ...
            input.value = '甇???...'; input.disabled = true;
...
def music_add(guild_id):
    ...
    async def do_add():
        ...
        # 璅⊥ AsyncContextManager for ctx.typing()
        class MockTyping:
        ...
        class MockCtx:
            def __init__(self, guild, author, bot):
                ...
                self.message = None # ???None嚗?撅祆批?????                # ?嚗芋?祈??喟???                self.author.voice = MockVoiceState(guild.voice_client.channel) if guild.voice_client else None
            
            def typing(self): return MockTyping()
            async def send(self, *args, **kwargs): 
                content = args[0] if args else kwargs.get('content', '')
                print(f"? [WebMusic] {content}")
                # 璅⊥?銝???臬?鞊?                class MockMsg:
        ...
        try:
            print(f"?儭?[WebMusic] 甇?暺?: {url}")
            # ?湔?? MusicCog ??play ?賣撠情
            ...
            if guild.voice_client and not guild.voice_client.is_playing():
                print("??[WebMusic] ?菜葫?圈?蝵殷??岫??閫貊??...")
                # ?ㄐ憒? play ?誘瘝葆 search ???曆?銝擐?(閬?music.py ?摩??)
                await play_cmd(music_cog, ctx, search=None)
        except Exception as e:
            import traceback
            print(f"??[WebMusic] ?游?航炊: {e}")
            traceback.print_exc()
