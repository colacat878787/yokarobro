...
async def transplant_context_menu(interaction: discord.Interaction, message: discord.Message):
    ...
    if not message.content and not message.embeds:
        ...
        return
    embed = discord.Embed(title="📋 訊息移植", description=message.content or "*（此訊息只有附件或 embed）*", color=0x3498db, timestamp=message.created_at)
    ...
    if message.embeds:
        embed.add_field(name="⚠️ 注意", value="原文包含 embed 內容，請查看原始訊息", inline=False)
...
async def clap_context_menu(interaction: discord.Interaction, message: discord.Message):
    ...
    embed = discord.Embed(title="👏 鼓掌時間！", description=f"{interaction.user.mention} 給 {message.author.mention} 鼓掌！", color=0xf1c40f)
...
async def quote_context_menu(interaction: discord.Interaction, message: discord.Message):
    """將訊息轉換為引用格式"""
    embed = discord.Embed(title="💬 引用訊息", description=message.content or "*（此訊息只有附件或 embed）*", color=0x95a5a6, timestamp=message.created_at)
    embed.set_author(name=f"{message.author.display_name} ({message.author})", icon_url=message.author.display_avatar.url)
